  ...
  implements OnGatewayConnection, OnGatewayInit, OnGatewayDisconnect
{
  ...
  handleEvent(
    ...
    switch (datajson.type) {
      case 'GETBOARD':
        client.emit(
          'MSGs',
          JSON.stringify(this.gameService.getBoard(datajson)),
        );
        break;
      ...
      case 'MOVE':
        const gameID = this.wsUsers.find(
          (el) => el.socketID === client.id,
        )?.gameID;
  ...
  public getConnectedClients() {
    const clientsList = Array.from(this.server.sockets.sockets.values()).map(
      (socket) => ({
        id: socket.id,
      }),
    );
